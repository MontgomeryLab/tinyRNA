...
# that is listed in versions array. It then installs tinyrna in each
...
for v in "${versions[@]}"; do
  ...
  conda create -n tinyrna-"$v" python="$v" --yes > /dev/null || exit

  # Install tinyrna. Read conda output for error strings in real-time
  conda env update -f ../environment.yml -n tinyrna-"$v" 2>&1 | while read line; do
    if [[ $line == "Found conflicts!"* || $line == "UnsatisfiableError"* ]]; then
      ...
      pid=$(ps -e -o pid,command | grep 'environment.yml -n tinyrna' | awk 'NR==1{print $1}')
      kill -INT "$pid"
    fi
  done

  # Activate the environment to verify intended Python version
  conda activate tinyrna-"$v"
